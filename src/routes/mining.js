import { mineApp, extractAllNgrams } from "../services/miningService.js";
...
export async function miningRoutes(fastify) {
   ...
   *
   * Competitor discovery via competitorsService.
   *
   * Body:
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      const results = await findCompetitors(fastify, appleId, stores);

      // Extract ngrams from competitors per store
      for (const storeResult of results) {
        if (storeResult.competitors?.length) {
          storeResult.ngrams = extractAllNgrams(storeResult.competitors);
        }
      }
