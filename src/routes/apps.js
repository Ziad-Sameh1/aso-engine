...
export async function appsRoutes(fastify) {
   ...
   * then resolve again.
   */
  async function resolveOrSearch(normKeyword, keyword, store, platform, { limit = 50 } = {}) {
    ...
    if (!kw) {
      ...
      kw = result.keywordId ? { id: result.keywordId } : await resolveKeyword(fastify.pg, normKeyword, store, platform);
    }
  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      if (!kw) return reply.code(404).send({ error: "Could not resolve keyword after search." });

      const row = await getAppCurrentRank(fastify.pg, appleId, kw.id);
      if (!row) return reply.code(404).send({ error: "App not found in search results for this keyword." });

      const result = { appleId, keyword, store, platform, rank: row.rank, rankedAt: row.ranked_at };
  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      const history = await getAppRankHistory(fastify.pg, appleId, kw.id, since);
  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      if (!row) {
        ...
        await insertSingleAppRating(fastify.pg, app.id, meta.rating, meta.ratingCount, store, platform);

        row = { rating: meta.rating, ratings_count: meta.ratingCount, recorded_at: new Date() };
      }
  ...
  fastify.get(
        ...
        response: {
          200: {
            ...
            properties: {
              ...
              cached:           { type: "boolean" },
              popularityBreakdown: { type: ["object", "null"], additionalProperties: true },
            },
          },
        },
      },
    ...
    async (request, reply) => {
      ...
      const kw = await resolveOrSearch(normKeyword, keyword, store, platform, { limit: 100 });
      if (!kw) return reply.code(404).send({ error: "Could not resolve keyword after search." });

      const [app, rankRow, popularityRow, competitivenessRow, popularityResult] = await Promise.all([
      ...
      const results = await Promise.all(
        keywords.map(async (keyword) => {
          ...
          try {
            ...
            const [rankRow, popularityRow, competitivenessRow, popularityResult] = await Promise.all([
              getAppCurrentRank(fastify.pg, appleId, kw.id),
              getKeywordCurrentPopularity(fastify.pg, kw.id),
              getKeywordCurrentCompetitiveness(fastify.pg, kw.id),
              calculatePopularity(normKeyword, store, platform, {
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      try {
        const data = await getKeywordSuggestions(fastify.pg, fastify.redis, appleId, { store, platform });
        ...
        fastify.log.error({ err, appleId }, "[POST /api/apps/:appleId/keywords/suggest] failed");
  ...
  fastify.get(
    ...
    async (request) => {
      ...
      const since = periodToDate(period);
      const history = await getAppRatingHistory(fastify.pg, appleId, store, platform, since);
